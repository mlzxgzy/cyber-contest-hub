...
- **项目可见性**：`GET /cch/project/list` 与 `GET /cch/project/{id}`（详情/成员列表）统一过滤——超管可见全部；普通用户仅可见自己创建（`t_project.create_by`）或作为成员（`t_project_member` 有记录，含邀请加入）的项目。非创建者且非成员的项目列表直接不显示，详情访问提示无权限。
