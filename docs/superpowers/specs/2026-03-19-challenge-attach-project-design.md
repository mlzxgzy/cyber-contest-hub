...
- 查询已附加列表：`cch:challenge:query`（挑战编辑者可看，无需项目成员身份），但按调用者可见性过滤结果——超管返回全部；普通用户仅返回自己创建或作为成员（含邀请加入）的项目中的附加记录，避免越权探测非可见项目
- 附加/移除：复用现有接口，需目标项目管理员权限（与项目侧一致）
- **项目可见性**：`GET /cch/project/list` 与 `GET /cch/project/{id}`（详情/成员列表）统一过滤——超管可见全部；普通用户仅可见自己创建（`t_project.create_by`）或作为成员（`t_project_member` 有记录且 `del_flag=0`，含邀请加入）的项目。非创建者且非成员的项目列表直接不显示，详情访问提示无权限。
